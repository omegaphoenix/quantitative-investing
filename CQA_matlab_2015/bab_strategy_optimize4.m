function weight = bab_strategy_optimize4(universe, selected_ticker, check_day)

N = length(selected_ticker);
assert(N==100)

beta_raw = calc_beta(universe.market.ret{1}, universe.stock.ret(selected_ticker), check_day,1/400);
...
for lambda = 10 
    ...
    w = [wL;-wS];
    maximize (-w'*beta - lambda*(w'*sigma*w))
    subject to
    sum(wL) == 1
    sum(wS) == 1
    max(wL) <= 0.04
